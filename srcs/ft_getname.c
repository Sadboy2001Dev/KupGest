...
int	ft_getname (char name[MAX_P][MAX_CHR])
{
	...
	while (i < MAX_P)
	{
		if (1)
		{
			...
			if (id[0] == -1)
				strcpy (name[0], n);
			// condition to fill to others
			// index (1,..., 9)
			else
				strcpy (name[count], n);
			// 1 will be returned if
			// everything goes well and
			// the name wasn't EMPTY
			return 1;
		}
		i++;
	}
	if (++i == MAX_P)
	{
	// This message will be showed
	// if all index was fully filled
		puts ("Limite excedido\n");
	// If all index was filled
	// the return will be 0
		return 0;                                 }
	// If everythings goes wrong
	// 0 will be returned
	return 0;
}
